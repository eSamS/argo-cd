...
func (c *clusterCache) replaceResourceCache(gk schema.GroupKind, resourceVersion string, objs []unstructured.Unstructured) {
	...
	if ok {
		...
		for key, existingNode := range c.resources {
...
func (c *clusterCache) stopWatching(gk schema.GroupKind) {
	c.lock.Lock()
	defer c.lock.Unlock()
...
func (c *clusterCache) watchEvents(ctx context.Context, api kube.APIResourceInfo, info *apiMeta, resClient dynamic.ResourceInterface) {
...
func (c *clusterCache) processApi(client dynamic.Interface, api kube.APIResourceInfo, callback func(resClient dynamic.ResourceInterface) error) error {
...
func (c *clusterCache) onNodeRemoved(key kube.ResourceKey, n *Resource) {
